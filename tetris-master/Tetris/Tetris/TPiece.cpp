...
TPiece::TPiece(const int a_x, const int a_y, const int a_IDColor, const int a_Size) :
	m_IDColor(a_IDColor),
	m_Size(a_Size)
{
...
TPiece::TPiece(const int a_x, const int a_y, int a_GridPosX, int a_GridPosY, const int a_IDColor, const int a_Size) :
...
void TPiece::CreatePiece(const int a_x, const int a_y, int a_GridPosX, int a_GridPosY)
{
	if (m_IDColor == 0)
	{
...
void TPiece::SetPosition(const int a_x, const int a_y, int GridPosX, int GridPosY)
{
	m_Tiles[0]->SetGridPosition(GridPosX, GridPosY - 1);
...
void TPiece::Draw(SDL_Surface* a_ScreenSurface)
{
...
void TPiece::Rotate(int a_angle, int a_OffsetX, int a_OffsetY)
{
	m_Angle = a_angle;
	...
	switch (m_Angle)
	{
		case 0:
			...
			m_Tiles[2]->SetGridPosition(m_Tiles[0]->GetGridPosX() + a_OffsetX, m_Tiles[0]->GetGridPosY());

			m_Tiles[0]->SetGridPosition(m_Tiles[0]->GetGridPosX() + 1 + a_OffsetX, m_Tiles[0]->GetGridPosY() - 1);
		...
		case 90:
			...
			m_Tiles[3]->SetGridPosition(m_Tiles[3]->GetGridPosX(), m_Tiles[3]->GetGridPosY() + a_OffsetY);
			break;
		...
		case 180:
			...
			m_Tiles[3]->SetGridPosition(m_Tiles[3]->GetGridPosX() + a_OffsetX, m_Tiles[3]->GetGridPosY());
			break;
...
vector<Tiles*> TPiece::GetTiles()
{
	return m_Tiles;
}

int TPiece::GetCollisionSize()
{
	return m_Size;
}
